    ...
    selectedProduct.length !== 0 && (
      <>
        <div className="flex flex-grow justify-end pb-5">
          <Select className="w-1/5 bg-white" size="small" value={rank} onSelect={(e)=>setRank(e.target.value)}>
            ...
            <div key={product.id}>
              <Card className="max-w-sm rounded overflow-hidden shadow-lg">
                ...
                <div className="px-5 pb-4 flex items-baseline">
                  <p className="text-xl font-bold pr-3">{`$${(
                    Number(product.price) -
                    Number(product.price).toFixed(1) *
                      (Number(product.discountPercentage).toFixed(0) / 100)
                  ).toFixed(1)}`}</p>
